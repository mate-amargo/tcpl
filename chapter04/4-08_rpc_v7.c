/*
 * Exercise 4-8. Supose that there will never be more than one character
...
char buf = EOF; /* Since there's only one character, there's no need for an
				   array. EOF indicates the buffer is empty */
...
{
	if (buf != EOF)
		printf("ungetch: there's already a char in the buffer\n");
	else
		buf = c;
}
